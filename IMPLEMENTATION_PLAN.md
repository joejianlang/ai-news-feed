...
### 目标
解决在全文模式下，“正在阅读”粘性工具栏遮挡文章配图或视频的问题。

### 问题分析
目前的“正在阅读”工具栏使用 `sticky` 布局。当文章向上滚动时，工具栏会吸附在分类栏下方。但在展开瞬间或特定滚动位置，由于 `sticky` 元素不占位或负边距等原因，它会悬浮在后续的图片/视频上方导致遮挡。

### 方案设计
1. **消除负边距**：在 `isFullExpanded` 模式下，移除内容容器的 `-mt-1` 或负边距。
2. **增加占位/内边距**：为内容容器增加合适的顶部间距（padding-top），确保工具栏在“非粘性状态”下也不覆盖内容。
3. **结构隔离**：确保工具栏和后续内容在同一个流中，且不使用绝对定位。
4. **两页同步**：在首页和关注页同步应用修改。
