...
### 目标
解决在全文模式下，“正在阅读”粘性工具栏覆盖文章配图的问题。

### 方案设计
1. **结构迁移**：将“正在阅读”工具栏从卡片外层移动到卡片内容容器（即包含图片和正文的 `div`）的最上方。
2. **消除重叠**：通过将工具栏置于内容流中，它将自然地占据空间并将图片推到下方，从而避免重叠。
3. **保持粘性**：继续保持 `sticky` 属性，并保留原有的 `top` 偏移量，确保它在滚动时能正确停留在分类栏下方。
