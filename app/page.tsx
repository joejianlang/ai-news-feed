...
function HomeContent() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <nav className="sticky top-[44px] sm:top-[64px] bg-black border-b border-zinc-900 z-40 shadow-sm transition-all duration-300">
        <div className="max-w-[1240px] mx-auto relative group">
          ...
          <div className="flex items-center overflow-x-auto scrollbar-hide px-3 py-3 sm:px-4 sm:py-3.5 gap-2 sm:gap-3 relative">
            <button
              onClick={() => setSelectedCategory(null)}
              className={`flex-shrink-0 px-5 py-2 sm:px-6 sm:py-2.5 text-[14px] sm:text-[15px] font-bold rounded-xl transition-all duration-300 ${selectedCategory === null
            ...
            {user && (
              <Link
                href="/following"
                className={`flex-shrink-0 px-5 py-2 sm:px-6 sm:py-2.5 text-[14px] sm:text-[15px] font-bold rounded-xl transition-all flex items-center gap-1.5 ${pathname === '/following'
                  ? 'bg-teal-600 text-white shadow-lg shadow-teal-500/20'
                  : 'bg-zinc-800 text-teal-400 hover:bg-zinc-700'
                  }`}
              >
                <span>关注</span>
              </Link>
            ...
            {categories
              .map((category: Category) => (
                <button
                  ...
                  className={`flex-shrink-0 px-5 py-2 sm:px-6 sm:py-2.5 text-[14px] sm:text-[15px] font-bold rounded-xl transition-all duration-300 ${selectedCategory === category.id
                    ? 'bg-teal-600 text-white shadow-lg shadow-teal-500/20'
                    : 'bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700'
                    }`}
                >
            ...
            {/* Arrow/More button placeholder style like in the image */}
            <button className="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-zinc-800 text-zinc-400 rounded-xl hover:text-white hover:bg-zinc-700 transition-all ml-1">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="m9 18 6-6-6-6" /></svg>
            </button>
          </div>
