    ...
    return (
      <div className="max-w-2xl mx-auto p-4 sm:p-8">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push('/')}
            className="bg-blue-500 text-white px-5 sm:px-6 py-2 rounded-lg hover:bg-blue-600 active:bg-blue-700 text-sm sm:text-base"
          >
            返回首页浏览内容
          </button>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4 sm:py-8">
      <div className="flex items-center justify-between mb-4 sm:mb-8">
        ...
        <button
          onClick={() => router.push('/')}
          className="text-blue-500 hover:text-blue-600 active:text-blue-700 text-sm sm:text-base"
        >
          ← 返回首页
        </button>
      </div>
        ...
        {news.map((item) => (
          <div key={item.id} className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            ...
            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-500 mb-3 flex-wrap">
              <span className="font-semibold text-blue-600">{item.source?.name}</span>
              {item.published_at && (
                <>
                  <span>•</span>
            ...
            <a
              ...
              className="text-blue-500 hover:text-blue-600 active:text-blue-700 text-xs sm:text-sm font-medium"
            >
              查看原文 →
            </a>
