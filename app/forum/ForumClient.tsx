...
export default function ForumClient() {
    ...
    const loadPosts = async () => {
        try {
            ...
            // 如果有深层链接但列表中没有，通过 API 单独拉取并插入到开头
            if (itemId && !finalPosts.some((p: ForumPost) => p.id === itemId)) {
                try {
                    const singleRes = await fetch(`/api/forum?id=${itemId}`);
                    const singleData = await singleRes.json();
                    if (singleData.posts && singleData.posts.length > 0) {
                        finalPosts = [singleData.posts[0], ...finalPosts];
                    }
                } catch (e) {
                    console.error('Failed to fetch deep link post:', e);
                }
            }
    ...
    // 处理深层链接
    useEffect(() => {
        if (itemId && posts.length > 0) {
            const post = posts.find(p => p.id === itemId);
            if (post) {
                setExpandedPostId(itemId);
                // 延迟滚动确保 DOM 已渲染
                setTimeout(() => {
                    const el = document.getElementById(`post-${itemId}`);
                    if (el) {
                        const headerHeight = 130; // 导航栏 + Header 的高度
            ...
            <div className="max-w-2xl mx-auto p-4 space-y-4">
                {loading ? (
                    ...
                    posts.map(post => {
                        ...
                        return (
                            <div key={post.id} id={`post-${post.id}`} className="bg-card rounded-3xl shadow-sm border border-card-border overflow-hidden transition-all duration-500">
                                <div className="p-6">
                                    {/* 作者与标签栏 */}
